		...
		if($event->getBlock()->getID() == Main::ITEM_ID){
			...
			if($chest instanceof Chest){
					...
					if($event->getPlayer()->hasPermission("chestlocker.bypass") == false){
						...
						if($this->plugin->isChestRegistered($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ())){
							if($this->plugin->getChestOwner($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ()) != strtolower($event->getPlayer()->getName())){
								...
								$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&4You not is The " . Main::ITEM_NAME_2 . " Owner ."));
							}
						}
					}
				}
				...
				if($this->plugin->getCommandStatus($event->getPlayer()->getName())==1){
					...
					if($this->plugin->isChestRegistered($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ())){
						if($this->plugin->getChestOwner($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ()) != strtolower($event->getPlayer()->getName())){
							$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&4You aren't the owner of this " . Main::ITEM_NAME_2 . "."));
						}else{
							$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&2" . Main::ITEM_NAME . " was locked"));
						}
					}else{
						$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&2" . Main::ITEM_NAME . " lock"));
				...
				if($this->plugin->getCommandStatus($event->getPlayer()->getName())==2){
					...
					if($this->plugin->isChestRegistered($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ())){
						if($this->plugin->getChestOwner($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ()) != strtolower($event->getPlayer()->getName())){
							$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&4You is not  " . Main::ITEM_NAME_2 . " Owner ."));
						}else{
							$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&2" . Main::ITEM_NAME . " Unlock"));
							$this->plugin->unlockChest($chest->getLevel()->getName(), $chest->getX(), $chest->getY(), $chest->getZ(), $event->getPlayer()->getName());
						}
					}else{
						$event->getPlayer()->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&2" . Main::ITEM_NAME . " Not Register"));
					}
					$event->setCancelled(true);
					$this->plugin->setCommandStatus(0, $event->getPlayer()->getName());
				}
				...
				if($this->plugin->isChestRegistered($level, $x, $y, $z)){
					...
					if($event->getPlayer()->hasPermission("chestlocker.bypass") == false){
						if($this->cfg["protect-chests"] == true){
							$player->sendMessage($this->plugin->SetColors("&", Main::PREFIX . "&4You is not " . Main::ITEM_NAME_2 . " owner ."));
							$event->setCancelled(true);
						}else{
